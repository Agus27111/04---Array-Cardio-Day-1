...
const liveLong = inventors.reduce((total, inventor) => {
...
const yearsLived = inventors.sort((a, b) => {
  ...
  return inventorB - inventorA; //longest to sortest
...
const alpha = people.slice().sort((a, b) => {
  //slice untuk menduplikat array nya agar orriginal array tetap tidak berubah
  const last = a.split(", ");
  const first = b.split(", ");
...
const sum = data.reduce((count, item) => {
  //reduce is used to iterate over an array and accumulate a single result or value by applying a specified callback
